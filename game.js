...
function getBossPatterns(waveNum) {
    const patterns = [
        { type: 'radial', interval: 12, bulletCount: 16 + waveNum * 4, speed: 2.5 + waveNum * 0.2, duration: 180 },
        { type: 'spiral', interval: 3, arms: 3 + Math.min(waveNum, 5), speed: 3 + waveNum * 0.15, duration: 240 },
        { type: 'flower', interval: 4, petals: 5 + Math.min(waveNum, 8), speed: 2.5 + waveNum * 0.2, duration: 200 },
        { type: 'aimed', interval: 8, spread: 5 + waveNum * 2, speed: 4 + waveNum * 0.3, duration: 150 },
        { type: 'random', interval: 4, count: 3 + waveNum, speed: 3 + waveNum * 0.2, duration: 180 },
    ];
...
function update() {
    ...
    if (boss && boss.alive) {
        if (boss.enterTimer > 0) {
            ...
            boss.patternTimer++;
            if (waveAnnounceDuration <= 0 && boss.patternTimer % pattern.interval === 0) {
